...
Person g() // RVO(Return Value Optimization)
{
	Person mother(2, "Jane"); // 생성자 호출
	return mother;			  // 복사 생성자 안됨
}
...
int main()
{
	...
	Person son = father; // 복사 생성자 호출 O
	f(father);			 // 복사 생성자 호출 O
	Person t = g();		 // 복사 생성자 호출 X
	t.show();
}